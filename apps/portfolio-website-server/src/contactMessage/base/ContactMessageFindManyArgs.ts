/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ContactMessageWhereInput } from "./ContactMessageWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ContactMessageOrderByInput } from "./ContactMessageOrderByInput";

@ArgsType()
class ContactMessageFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ContactMessageWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ContactMessageWhereInput, { nullable: true })
  @Type(() => ContactMessageWhereInput)
  where?: ContactMessageWhereInput;

  @ApiProperty({
    required: false,
    type: [ContactMessageOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ContactMessageOrderByInput], { nullable: true })
  @Type(() => ContactMessageOrderByInput)
  orderBy?: Array<ContactMessageOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ContactMessageFindManyArgs as ContactMessageFindManyArgs };
